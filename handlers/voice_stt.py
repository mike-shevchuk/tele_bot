...
async def handle_voice(message: types.Message, bot: Bot, logger):
    ...
    logger.info(f"STT request from {message.from_user.id}, suffix={suffix}")

    wait_msg = await message.reply("Транскрибую аудіо...", disable_notification=True)

    try:
        tg_file = await bot.get_file(file_id)
        logger.info(f"TG file: {tg_file.file_path}, size={tg_file.file_size}")
        ...
        if not await _check_health(logger):
            await wait_msg.edit_text(f"STT сервіс недоступний ({STT_HOST}:{STT_PORT})")
            return

        logger.info(f"Sending to {STT_URL}?language={STT_LANG}&model={STT_MODEL}")
        resp = await _transcribe(audio_bytes, suffix)
        logger.info(f"STT response: status={resp.status_code}, content-length={len(resp.content)}")
        ...
        logger.error(f"STT connect failed: {STT_URL} -> {e}")
        await wait_msg.edit_text(f"STT сервіс недоступний ({STT_HOST}:{STT_PORT})")
    except (httpx.RemoteProtocolError, httpx.ReadError) as e:
        logger.exception(f"STT disconnect: url={STT_URL}, audio_size={len(audio_bytes) if 'audio_bytes' in dir() else '?'}, error={e}")
